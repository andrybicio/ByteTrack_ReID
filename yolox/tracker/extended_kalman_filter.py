...
class KalmanFilter(object):
    ...
    The 12-dimensional state space
        ...
        x, y, a, h, vx, vy, va, vh, ax, ay, vx, vy 

    contains the bounding box center position (x, y), aspect ratio a, height h,
    and their respective velocities.

    Object motion follows a constant acceleration model. The bounding box location
    ...
    def __init__(self):
        ...
        # Create Kalman filter model matrices. Second order filter
        self._motion_mat = np.eye(3 * ndim, 3 * ndim)
        for i in range(ndim):
            self._motion_mat[i, ndim + i] = dt
            self._motion_mat[i + ndim, 2*ndim + i] = dt
        for i in range(int(ndim/2)): # only the x and y 
    ...
    def initiate(self, measurement):
        ...
        (ndarray, ndarray)
            ...
            dimensional) of the new track. Unobserved velocities and accelerations are initialized
            to 0 mean.
        ...
        std = [
            ...
            1e-5,
            10 * self._std_weight_velocity * measurement[3],
            25 * self._std_weight_acceleration * measurement[3],
            25 * self._std_weight_acceleration * measurement[3],
            5e-7,
            25 * self._std_weight_acceleration * measurement[3]]
    ...
    def multi_predict(self, mean, covariance):
        ...
        ----------
        mean : ndarray
            The Nx12 dimensional mean matrix of the object states at the previous
            time step.
        covariance : ndarray
            The Nx12x12 dimensional covariance matrics of the object states at the
            previous time step.
        Returns
        -------
        (ndarray, ndarray)
